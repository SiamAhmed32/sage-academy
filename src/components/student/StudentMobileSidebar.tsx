...
type StudentMobileSidebarProps = {
  user: AuthUser;
  student: StudentShellStudent | null;
};

export function StudentMobileSidebar({ user, student }: StudentMobileSidebarProps) {
    ...
    <Sheet>
      ...
      >
        <div className="shrink-0 border-b border-sage-white/10 p-5">
          <StudentSidebarHeader user={user} student={student} compact />
        </div>
                    ...
                    return (
                      <SheetClose asChild key={item.href}>
                        <Link
                          href={item.href}
                          className={cn(
                            "flex items-center gap-3 rounded-lg px-4 py-3 text-sm font-semibold text-sage-white/75 transition hover:bg-white/10 hover:text-white",
