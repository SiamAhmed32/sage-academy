...
export function AdmissionLeadPicker({ onSelect, onClear, selectedName }: Props) {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admission-requests?${params.toString()}`, {
    ...
    return (
      <div className="flex items-center gap-3 rounded-xl border border-green-200 bg-green-50 px-4 py-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-bold text-green-700 uppercase tracking-wider">Applicant Selected</p>
  ...
  return (
    <div ref={ref} className="relative">
      <div className="flex items-center gap-3 rounded-xl border border-dashed border-sage-border bg-sage-red-50/20 px-4 py-3">
        ...
        <input
          ...
          placeholder={
            deferredQuery !== query
              ? "Searching..."
              : "Search by applicant name or phone number..."
          }
          value={query}
      ...
      {open && deferredQuery === query && query && leads.length === 0 && (
