        ...
        <AnimatePresence mode="wait">
          ...
          >
            {visibleTestimonials.map((item, index) => (
              <TestimonialCard key={item._id} item={item} index={index} />
            ))}
          </motion.div>
        </AnimatePresence>
      </Container>
...
function NavBtn({ icon, onClick }: { icon: React.ReactNode; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="flex h-12 w-12 items-center justify-center rounded-2xl border border-sage-warm-border bg-sage-white text-sage-secondary shadow-sm transition hover:-translate-y-0.5 hover:border-sage-primary hover:text-sage-primary"
    >
      {icon}
    </button>
  );
