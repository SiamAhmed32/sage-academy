...
export async function uploadStudentImage(file: File) {
  ...
  try {
    const relativeDir = path.join("uploads", "students");
    const targetDir = path.join(process.cwd(), "public", relativeDir);
    await mkdir(targetDir, { recursive: true });
    const filename = `${Date.now()}-${randomUUID()}.${extension}`;
    const fullPath = path.join(targetDir, filename);
    await writeFile(fullPath, bytes);
    return { url: `/${relativeDir.replace(/\\/g, "/")}/${filename}`, publicId: "" };
  } catch {
    throw new BadRequestError("Student image upload failed");
  }
}
